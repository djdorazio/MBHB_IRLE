...
def Fnuint_Ring_Iso(ph, t, Dist, args, RHStable, Ttable):
	Lavg, Amp, Ombin, t0, n0, Rd, p, thetT, JJ, aeff, nu0, nn = args
...
## Coordinates of the dust Ring
	...
	Th_ring = np.arccos( np.cos(ph) * np.sin(JJ) ) 
	cosTh_ring = np.cos(ph) * np.sin(JJ)  

	## retarded time - time light emitted from dust
	#tem = t - Rd/c*( 1. - np.sin(Th_ring)*np.cos(ph) )   #J=pi/2 is face on
	if (JJ<np.pi/4):
		...
		tem = t - Rd/c*( 1. - cosTh_ring  )   #J=0 is face on


	
	# Tdust = TDust_Ring_Iso(tem, Rd, Th_ring, ph, args, RHStable, Ttable)
	...
	fint = Lavg*(1. + Amp * np.sin( Ombin*(tem) ))
	#fint = fint* Rd*Rd* np.sin(Th_ring) * l_d 
	fint = fint* Rd*Rd* np.sqrt(1 - cosTh_ring*cosTh_ring) * l_d 
...
def Fnuint_Thick_Iso_PG(ph, thet, r, nu, t, Dist, args, RHStable, Ttable): #, tauGrid):
	Lavg, Amp, Ombin, t0, n0, Rd, p, thetT, JJ, aeff, nu0, nn = args
	## where UV penetrates out to (tau_UV=1)
	Rout =  Rd*(  1. - 3.*(p - 1.)/(n0*ma.pi*aeff*aeff*Rd)  )**(1./(1. - p)) #+ 0.01*Rd
...
#starting point of secindary at t=0
	...
	#FOR SIMPLICITY start with (vx,vy) = bet(cosOMt, sinOmt) and rotate by Inc in clockwise direction so taht Inc is positive
	phis = ma.pi
	Vxorb = np.cos(incl) * np.cos(phis + Ombin*t) * np.sin(thetas)
	Vyorb = np.sin(phis + Ombin*t) * np.sin(thetas)
	Vzorb = np.sin(incl)* np.cos(phis + Ombin*t) * np.sin(thetas)
...
def TDust_Ring_Dop(t,r,thet,phi, args, RHStable, Ttable):
	Lavg, bets, incl, Ombin, alphnu, n0, Rd, p, thetT, JJ, aeff, nu0, nn = args
...
## retarded time - time light emitted form dust
	if (JJ<np.pi/4):
		...
		tem = t - Rd/c*( 1. - cosTh_ring  ) 
	# Tdust for ISO source
	#args[8] = 0.0 #set thea_T = 0 for Ring/Sphere
	
	Tdust = TDust_Ring_Dop(tem, Rd, Th_ring, ph, args, RHStable, Ttable)
	# surface density in optically thick limit
	l_d = (2.*aeff)*(2.*aeff)/(4./3. * ma.pi * aeff*aeff*aeff)
	# Rd is the inner edge of the shell
	# fint =  2.*h*nu*nu*nu/(c*c)*1./(np.exp(  h*nu/(kb*Tdust)  ) - 1.)	
...
###----------------------------###
	...
	fint = 0.0#1.e-20
	# if ndust is zero on the near side of the dust, then tau=0
	if (nDust(xe,y,z, n0, Rd, p, thetT, JJ) <= 0.0000000001 and x<=0.0):
		#fint = Qv(nu, nu0, nn) * np.exp(-tauobs) * 2.*h*nu*nu*nu/(c*c)*1./(np.exp(  h*nu/(kb*Tdust)  ) - 1.)	
		fint = Qv(nu, nu0, nn)  * 2.*h*nu*nu*nu/(c*c)*1./(np.exp(  h*nu/(kb*Tdust)  ) - 1.)	
		fint = fint* Rd*Rd* np.sin(thet) * Surf_nd
...
def F_Sphere_Iso_TrapInt(numin, numax, t, Dist, Aargs, RHStable, Ttable):
	...
	# 		Trap_sub = np.exp(lnnus[0])*Fnu_Sphere_Iso_TrapInt(np.exp(lnnus[0]), t[i], Dist, Aargs, RHStable, Ttable) + np.exp(lnnus[Ntrap_nu-1])*Fnu_Sphere_Iso_TrapInt(np.exp(lnnus[Ntrap_nu-1]), t[i], Dist, Aargs, RHStable, Ttable)
	# 		Trap_int = 0.0
	# 		for j in range(Ntrap_nu):
	# 			Trap_int = -Trap_sub + 2.*np.exp(lnnus[j])*Fnu_Sphere_Iso_TrapInt(np.exp(lnnus[j]), t[i], Dist, Aargs, RHStable, Ttable)

	# 		#Trap_int = -Trap_sub + ma.fsum(2.*np.exp(lnnus)*Fnu_Sphere_Iso_TrapInt(np.exp(lnnus), t[i], Dist, Aargs, RHStable, Ttable))
	# 		Trap_int = Trap_int - Trap_sub
...
def F_Ring_Iso_QuadInt(t, Dist, Aargs, RHStable, Ttable):
 	return intg.quad(Fnuint_Ring_Iso, 0.,2.*ma.pi, args=(t, Dist, Aargs, RHStable, Ttable),  full_output=fo  )[0]
...
def FThnu_Sphere_Dop_TrapInt(thet, nu, t, Dist, Aargs, RHStable, Ttable):
	...
	Trap_int = -Trap_sub +  np.sum(2.*Fnuint_Shell_OptThin_Dop(phis,thet, nu, t, Dist, Aargs, RHStable, Ttable)) 
	return 2.*ma.pi/(2.*Ntrap_ph) * (Trap_int)
...
def F_Sphere_Dop_TrapInt(numin, numax, t, Dist, Aargs, RHStable, Ttable):
	...
	#nus = np.log(nus)
	#if (type(t) is float or type(t) is np.float64):
	Trap_sub = nus[0]*Fnu_Sphere_Dop_TrapInt(nus[0], t, Dist, Aargs, RHStable, Ttable) + nus[Ntrap_nu-1]*Fnu_Sphere_Dop_TrapInt(nus[Ntrap_nu-1], t, Dist, Aargs, RHStable, Ttable)
	Trap_int = -Trap_sub + sum(2.*nus*Fnu_Sphere_Dop_TrapInt(nus, t, Dist, Aargs, RHStable, Ttable) )

	return (numax-numin)/(2.*Ntrap_nu) * (Trap_int)
	# else:
	# 	res = []
	# 	for i in range(len(t)):
	# 		Trap_sub = nus[0]*Fnu_Sphere_Dop_TrapInt(nus[0], t[i], Dist, Aargs, RHStable, Ttable) + nus[Ntrap_nu-1]*Fnu_Sphere_Dop_TrapInt(nus[Ntrap_nu-1], t[i], Dist, Aargs, RHStable, Ttable)
	# 		Trap_int = -Trap_sub + sum(2.*nus*Fnu_Sphere_Dop_TrapInt(nus, t[i], Dist, Aargs, RHStable, Ttable) )
...
def F_Ring_Dop_QuadInt(t, Dist, Aargs, RHStable, Ttable):
	return intg.quad(Fnuint_Ring_Dop, 0.,2.*ma.pi, args=(t, Dist, Aargs, RHStable, Ttable),  full_output=fo  )[0]
