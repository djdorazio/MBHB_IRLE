...
Sinp0_W1 = [0.1, 365*5., 1.0, 11.3]
Sinp0_W2 = [0.1, 365*5., 1.0, 10.3]
...
t_avg.append(np.mean(t_MJD[0:iseg[0]]))
W1_avg.append(np.mean(W1_mag[0:iseg[0]]))
W2_avg.append(np.mean(W2_mag[0:iseg[0]]))

W1_avsg.append((max(W1_mag[0:iseg[0]]) - min(W1_mag[0:iseg[0]]))/10.) ### MAKE ERR SMALL TEMP
W2_avsg.append((max(W2_mag[0:iseg[0]]) - min(W2_mag[0:iseg[0]]))/10.)
...
W2_avg.append(np.mean(W2_mag[iseg[len(iseg)-1]:len(t_MJD)]))

W1_avsg.append((max(W1_mag[iseg[len(iseg)-1]:len(t_MJD)]) - min(W1_mag[iseg[len(iseg)-1]:len(t_MJD)]))/10.)
W2_avsg.append((max(W2_mag[iseg[len(iseg)-1]:len(t_MJD)]) - min(W2_mag[iseg[len(iseg)-1]:len(t_MJD)]))/10.)
...
if (SinFit):
	...
	W2_sin_walker_p0 = np.random.normal(W2_sin_p0, np.abs(W1_sin_p0)*1E-4, size=(nwalkers, ndim))
